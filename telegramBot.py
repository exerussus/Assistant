...
class TelegramBot:
    ...
    def __init__(self, telegram_token=Config().telegram.token, openai_token=Config().openai.token):
    ...
    def is_it_app_check(self, message):
        ...
        for app in self.apps_list:
            if _text in app.activate_words:
                return app
        ...
        try:
            if self.user_access_to_app_check(user_id=user_id, app_name=app.name):
                ...
                self.bot.send_message(message.from_user.id,
                                      f"id: {user_id}. У вас нет прав на использование данной команды.")
                clean_user_status(user_id=user_id)
        except AttributeError:
            set_user_status(user_id=user_id, actually_app="", scenario="")
            print("Ошибка приложения или сценария. Сброс состояния пользователя.")
            ...
            if app is not None:
                ...
                if get_access_rights(user_id=user_id, app_name="chatGPT"):
                    self.bot.send_message(user_id,
                                          self.chat_bot_dict[user_id].response(message),  # Переделать
    ...
    def run(self):
        ...
        def get_text_messages(message):
            messageTime = datetime.datetime.utcfromtimestamp(message.date)
            now = datetime.datetime.utcnow()
            delta = now - messageTime
